---
description: Scopes Conventional Commits, Commitlint, Husky, staged SemVer validation, release signals, and Git quality gates for Orbu.
globs:
  - "package.json"
...
4. Husky hooks remain executable thin shell adapters to Orbu; business logic stays in `cli/src/commands/`.
...
10. Committed-history validation (`orbu git lint` and `orbu git commits`) checks every non-merge commit in the requested range. Git parent topology identifies integration merge envelopes; their generated messages are excluded, regardless of subject wording. A single-parent commit with a merge-looking subject remains fully validated.
