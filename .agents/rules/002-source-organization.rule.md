---
description: Scopes source organization, concern folders, naming, suffixes, and colocated-test placement for the Orbo TypeScript package.
globs:
  - "src/**"
  - "tsconfig*.json"
  - "tsdown*.ts"
...
5. No file or directory below `src/` may begin with `orbo` or `orbo-`, except the owner-selected canonical configuration file `src/orbo.config.json` (ADR-0013, identity updated by ADR-0022).
