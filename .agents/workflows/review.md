...
Use for a pre-merge review of OrbV code, package, or harness changes.
...
8. Run `orbv lint`, `orbv typecheck`, and `orbv test` when reviewing executable changes.
9. Run `orbv check` before declaring the change release-ready.
...
audit with `sh .audits/documentation.audit.sh`, then the complete OrbV gate. Record
