...
Use for a pre-merge review of Orb Voice code, package, or harness changes.
...
8. Run `orb-voice lint`, `orb-voice typecheck`, and `orb-voice test` when reviewing executable changes.
9. Run `orb-voice check` before declaring the change release-ready.
...
audit with `sh .audits/documentation.audit.sh`, then the complete Orb Voice gate. Record
