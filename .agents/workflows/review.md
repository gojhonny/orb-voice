...
Use for a pre-merge review of Orbu code, package, or harness changes.
...
8. Run `orbu lint`, `orbu typecheck`, and `orbu test` when reviewing executable changes.
9. Run `orbu check` before declaring the change release-ready.
...
audit with `sh .audits/documentation.audit.sh`, then the complete Orbu gate. Record
