...
is included only to provide the `orbv` package binary and explicit npx project
setup.
...
A release-oriented source change runs `orbv check`; CI also lints commit history
through the checked-in OrbV entry point and runs `npm pack --dry-run`. The
`prepack` lifecycle delegates to the same OrbV gate. New exports, attributes,
...
`package.json#scripts` does not mirror the OrbV command surface. A root-only local
...
setup, engineering commands use `orbv <command>` directly.

Consumer project setup remains explicit through
`npx orbv --setup`. Standard dependency lifecycles
...
ADR-0021/SPEC-030 make `orbv` the canonical npm identity while GitHub metadata
targets `gojhonny/orbv`. ADR-0018/SPEC-027 are historical records of the previous
...
until then, examples can select NeonGate through `DEFAULT_ORBV_PRESET` or omit
...
Agent runtime guardrails deny autonomous package publication and require human approval for tag, push, merge/rebase, and PR-merge boundaries. These hooks supplement, but do not replace, OrbV checks, Git hooks, and CI.
