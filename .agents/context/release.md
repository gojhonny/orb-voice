...
is included only to provide the `orbu` package binary and explicit npx project
setup.
...
A release-oriented source change runs `orbu check`; CI also lints commit history
through the checked-in Orbu entry point and runs `npm pack --dry-run`. The
`prepack` lifecycle delegates to the same Orbu gate. New exports, attributes,
...
`package.json#scripts` does not mirror the Orbu command surface. A root-only local
...
setup, engineering commands use `orbu <command>` directly.

Consumer project setup remains explicit through
`npx orbu --setup`. Standard dependency lifecycles
...
ADR-0023/SPEC-032 make `orbu` the npm package, public CLI, and
`<orb-u>` element. GitHub metadata targets `gojhonny/orbu`.
ADR-0022/SPEC-031 record the previous distribution identity.
...
until then, examples can select NeonGate through `DEFAULT_ORBU_PRESET` or omit
...
Agent runtime guardrails deny autonomous package publication and require human approval for tag, push, merge/rebase, and PR-merge boundaries. These hooks supplement, but do not replace, Orbu checks, Git hooks, and CI.
