...
is included only to provide the `orb-voice` package binary and explicit npx project
setup.
...
A release-oriented source change runs `orb-voice check`; CI also lints commit history
through the checked-in Orb Voice entry point and runs `npm pack --dry-run`. The
`prepack` lifecycle delegates to the same Orb Voice gate. New exports, attributes,
...
`package.json#scripts` does not mirror the Orb Voice command surface. A root-only local
...
setup, engineering commands use `orb-voice <command>` directly.

Consumer project setup remains explicit through
`npx orb-voice --setup`. Standard dependency lifecycles
...
until then, examples can select NeonGate through `DEFAULT_ORB_VOICE_PRESET` or omit
...
Agent runtime guardrails deny autonomous package publication and require human approval for tag, push, merge/rebase, and PR-merge boundaries. These hooks supplement, but do not replace, Orb Voice checks, Git hooks, and CI.
