...
- `orbv` is side-effect free and safe to import during SSR.
- `orbv/browser` registers `<orb-v>` in the active custom-element registry.
- `orbv/react-types` provides type-only JSX augmentation.
- `orbv/standalone` is the direct-browser bundle.
- `orbv/index.css` exposes package CSS when explicitly needed.
...
`src/orbv.config.json` contains editable component, palette and realtime settings.
...
`OrbVVoiceEnginePort` handles output-only speech; `OrbVConversationPort` handles
