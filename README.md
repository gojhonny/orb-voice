<p align="center">
  <img
    src="./assets/images/orbv-tagline.svg"
    alt="One native voice-presence component for every web stack."
  >
</p>

<p align="center">
  <img src="./assets/images/readme-banner.png" alt="OrbV voice presence component" width="100%">
</p>
...
<p align="center">
  ...
  <a href="https://github.com/gojhonny/orbv/actions/workflows/ci.yml"><img alt="Tests" src="https://img.shields.io/github/actions/workflow/status/gojhonny/orbv/ci.yml?branch=main&label=tests&logo=github" height="20"></a>
  <a href="https://www.npmjs.com/package/orbv"><img alt="npm version" src="https://img.shields.io/npm/v/orbv?logo=npm" height="20"></a>
</p>

<p align="center">
  <a href="https://neongate.com.br/docs/orbz/overview"><strong>Documentation</strong></a>&nbsp;&nbsp;&nbsp;
  <a href="https://www.npmjs.com/package/orbv"><strong>npm</strong></a>&nbsp;&nbsp;&nbsp;
...
`orbv` is a framework-agnostic, SSR-safe Web Component for AI voice
...
OrbV renders the native `<orb-v>` element. Your application keeps ownership of
the persona, transcript UI, authentication, backend, memory, tools, quotas, and
product logic.

| Capability | What OrbV provides |
| --- | --- |
| Native Web Component | One `<orb-v>` element for React, Next.js, Vue, Svelte, Angular, vanilla JS, and mixed stacks |
...
Register `<orb-v>` from browser-only code:
...
```ts
import type { OrbVElement } from 'orbv'
import 'orbv/browser'

const orb = document.querySelector<OrbVElement>('orb-v')!
...
OrbV keeps visual presence and voice transport separate from your application's
...
Selecting a model is silent. OrbV does not automatically start playback or open
...
Language selection controls voice matching and pronunciation. OrbV does not
...
For the endpoint form, OrbV posts JSON containing `{ sdp, model, voice }` and
...
small `OrbVVoiceEnginePort` contract:

```ts
import type { OrbVVoiceEnginePort } from 'orbv'

class MyVoiceEngine implements OrbVVoiceEnginePort {
...
OrbV ships without a persona, greeting, fallback copy, or hidden conversation
...
boundary, not an embedded OrbV LLM or credential store.
...
```html
<orb-v state="thinking"></orb-v>
```

When OrbV speaks through its talk API, it temporarily switches to `speaking` and
...
```html
<orb-v preset="peach" state="listening"></orb-v>
```
...
OrbV dispatches native `CustomEvent` instances from the host element.

| Event | Detail |
| --- | --- |
| `orbv-conversation-state-change` | `{ state }` where state is `idle`, `connecting`, `listening`, `thinking`, `speaking`, or `error` |
| `orbv-transcript` | `{ role, text, final, itemId? }` for user/assistant transcript updates |
| `orbv-speaking-change` | `{ speaking }` for audible output transitions |
| `orbv-talk-error` | `{ error }` for sanitized built-in talk/provider errors |

```ts
orb.addEventListener('orbv-transcript', (event) => {
...
OrbV does not store transcript history, conversation memory, or audio history.
...
OrbV remains a native custom element. React projects can add JSX typing without
using a wrapper component:

```ts
import 'orbv/react-types'
import 'orbv/browser'
```
...
closed shadow tree. Use OrbV appearance APIs for the component itself and wrap
...
```ts
await import('orbv/browser')
```

If you prefer explicit registration instead of the browser side-effect entry:

```ts
import { defineOrbV } from 'orbv'

defineOrbV()
```

`defineOrbV()` defines `<orb-v>` once and safely returns without registering in
...
OrbV accepts public model configuration and an application authorization
...
| OrbV owns | Your application owns |
| --- | --- |
...
- For a decorative orbv, hide the host from assistive technology.
...
| Import | Purpose |
| --- | --- |
| `orbv` | Types, constants, factories, ports, adapters, and explicit registration API |
| `orbv/browser` | Main API plus automatic browser registration |
| `orbv/react-types` | React JSX type augmentation |
| `orbv/standalone` | Direct-browser/CDN bundle |
| `orbv/index.css` | Explicit stylesheet export |

The README is intentionally focused on **using OrbV in an application**. Deeper
...
<p align="center">
  <a href="https://neongate.com.br/docs/orbz/overview"><strong>Read the documentation →</strong></a>
</p>
