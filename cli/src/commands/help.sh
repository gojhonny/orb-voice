  ...
  case "$orb_help_command" in
    git)
      ...
      case "$orb_help_subcommand" in
        ''|help|--help|-h)
          [ "$#" -eq 0 ] || orb_die 'Usage: orbo help git [subcommand]' 2
          cat <<'EOF_GIT'
Usage: orbo git [--logs] <command> [--logs] [arguments]
...
Run 'orbo help git <command>' or 'orbo git <command> --help' for details.
EOF_GIT
          exit 0
          ;;
        commit)
          ...
          [ "$#" -eq 0 ] || orb_die 'Usage: orbo help git commit [message]' 2
          printf 'Usage: orbo git commit message <message-file>\n'
          exit 0
          ;;
        setup|doctor|pre-commit|commit-message|commit-msg|lint|commits|version-check)
          [ "$#" -eq 0 ] || orb_die "Usage: orbo help git $orb_help_subcommand" 2
          exec "$ORB_CLI_DIR/orb.sh" git "$orb_help_subcommand" --help
          ;;
        *) orb_die "Unknown git help topic: $orb_help_subcommand. Run 'orbo help git'." 2 ;;
      esac
      ;;
    bootstrap|install|setup|--setup|doctor|cleanup|clean|lint|typecheck|test|build|harness|neon|audit|check|version|--version|-V)
      [ "$#" -eq 0 ] || orb_die "Usage: orbo help $orb_help_command" 2
      exec "$ORB_CLI_DIR/orb.sh" "$orb_help_command" --help
      ;;
    help|--help|-h)
      [ "$#" -eq 0 ] || orb_die 'Usage: orbo help [command]' 2
      ;;
    *) orb_die "Unknown help topic: $orb_help_command. Run 'orbo help'." 2 ;;
  esac
fi
...
Options:
  ...
  --package-spec <specifier>            Override the Orbo package/version to install
  --force                               Reinstall even when Orbo is already declared
  ...
  --version, -V                         Print the executing Orbo version

The default command is setup. It adds orbo-voice to an existing project
...
cat <<'EOF_REPOSITORY'
Orbo repository engineering CLI

Usage:
  orbo [--logs] <command> [arguments]
  ./cli/orbo <command> [arguments]
  orbo help <command>
  orbo help git <command>

Commands:
  help [command]                        Show this guide or command-specific help
  --version                             Print the local Orbo version
  bootstrap                             Install dependencies and configure the checkout
  setup [--launcher] [--bin-dir <dir>]  Install the user-scoped orbo launcher
  setup --project <dir> [options]       Install Orbo into an existing consumer project
  doctor [--ci]                         Diagnose the Orbo engineering environment
...
Orbo is implemented entirely with POSIX shell scripts. Package commands are
owned by Orbo; package.json keeps only the setup bridge and npm lifecycle gates.
EOF_REPOSITORY
