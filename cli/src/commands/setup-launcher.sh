...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      ...
      cat <<'EOF'
Usage:
  orb-voice setup [--launcher] [--bin-dir <directory>]

Destination precedence:
  --bin-dir, ORB_BIN_DIR, PNPM_HOME, XDG_BIN_HOME, ~/.local/bin

Set ORB_SETUP_DISABLED=1 to skip launcher installation. Orb Voice never edits a
...
case "${ORB_SETUP_DISABLED:-0}" in
  1|true|TRUE|yes|YES)
    printf 'Orb Voice launcher setup skipped: ORB_SETUP_DISABLED is set.\n'
    exit 0
    ;;
esac

if [ "$orb_setup_mode" = bootstrap ] && orb_ci_enabled; then
  printf 'Orb Voice launcher setup skipped in CI.\n'
  exit 0
fi
...
orb_target="$orb_bin_dir/orb-voice"
if [ -L "$orb_target" ]; then
  ...
  [ "$orb_marker" = '# managed-by: orb-voice' ] || orb_die "Unmanaged command already exists: $orb_target"
fi
...
if [ ! -x "$fallback_root/cli/orb-voice" ]; then
  printf 'orb-voice: configured Orb Voice checkout is unavailable: %s\n' "$fallback_root" >&2
  printf 'orb-voice: run ./cli/orb-voice setup from a valid checkout\n' >&2
  exit 2
fi

exec "$fallback_root/cli/orb-voice" "$@"
EOF
} >"$orb_tmp"
chmod 755 "$orb_tmp"

if [ -x "$orb_target" ] && cmp -s "$orb_tmp" "$orb_target"; then
  printf 'Orb Voice launcher already configured at %s\n' "$orb_target"
  exit 0
fi
...
printf 'Orb Voice launcher installed at %s\n' "$orb_target"
