...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      ...
      cat <<'EOF'
Usage:
  orbu setup [--launcher] [--bin-dir <directory>]

Destination precedence:
  --bin-dir, ORB_BIN_DIR, PNPM_HOME, XDG_BIN_HOME, ~/.local/bin

Set ORB_SETUP_DISABLED=1 to skip launcher installation. Orbu never edits a
...
case "${ORB_SETUP_DISABLED:-0}" in
  1|true|TRUE|yes|YES)
    printf 'Orbu launcher setup skipped: ORB_SETUP_DISABLED is set.\n'
    exit 0
    ;;
esac

if [ "$orb_setup_mode" = bootstrap ] && orb_ci_enabled; then
  printf 'Orbu launcher setup skipped in CI.\n'
  exit 0
fi
...
if [ -L "$orb_target" ]; then
  ...
  [ "$orb_marker" = '# managed-by: orbu' ] || orb_die "Unmanaged command already exists: $orb_target"
fi
...
if [ ! -x "$fallback_root/cli/orbu" ]; then
  printf 'orbu: configured Orbu checkout is unavailable: %s\n' "$fallback_root" >&2
  printf 'orbu: run ./cli/orbu setup from a valid checkout\n' >&2
  exit 2
fi

exec "$fallback_root/cli/orbu" "$@"
EOF
} >"$orb_tmp"
chmod 755 "$orb_tmp"

if [ -x "$orb_target" ] && cmp -s "$orb_tmp" "$orb_target"; then
  printf 'Orbu launcher already configured at %s\n' "$orb_target"
  exit 0
fi
...
printf 'Orbu launcher installed at %s\n' "$orb_target"
