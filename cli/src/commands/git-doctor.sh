...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      ...
      printf 'Usage: orbo git doctor [--ci]\n'
      exit 0
      ;;
...
if grep -F 'orbo git pre-commit' "$ORB_PROJECT_ROOT/.husky/pre-commit" >/dev/null 2>&1; then
  pass 'pre-commit is a thin Orbo adapter'
else
  fail 'pre-commit is not a thin Orbo adapter'
fi
if grep -F 'orbo git commit-message' "$ORB_PROJECT_ROOT/.husky/commit-msg" >/dev/null 2>&1; then
  pass 'commit-msg is a thin Orbo adapter'
else
  fail 'commit-msg is not a thin Orbo adapter'
fi
...
if [ "$ci" = false ] && orb_has git && orb_git_checkout; then
  ...
  if [ "$hooks_path" = .husky/_ ]; then
    ...
    fail "core.hooksPath is '${hooks_path:-unset}'; run orbo git setup"
  fi
fi
