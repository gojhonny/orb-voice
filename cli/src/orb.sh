...
orb_usage_error() {
  orb_print_error "Orbo: $1"
  printf "Run 'orbo help' for usage.\n" >&2
  exit 2
}
    ...
    case "$orb_subcommand" in
      ...
      commit)
        case "${1:-}" in
          ...
          message) ;;
          *) orb_usage_error 'Usage: orbo git commit message <message-file>' ;;
        esac
        shift
      ...
      *) orb_usage_error 'Usage: orbo git <setup|doctor|pre-commit|commit-message|lint|commits|version-check>' ;;
    esac
    ;;
