...
orb_die() {
  orb_message=$1
  orb_status=${2:-1}
  orb_print_error "OrbV: $orb_message"
  if [ "$orb_status" -eq 2 ] && [ -n "${ORB_HELP_TOPIC:-}" ]; then
    printf "Run 'orbv help %s' for usage.\n" "$ORB_HELP_TOPIC" >&2
  fi
  exit "$orb_status"
}
...
orb_lint_commit_history() {
  orb_need git
  orb_need pnpm
  orb_git_checkout || orb_die 'Commit history validation must run inside the OrbV checkout.'
  cd "$ORB_PROJECT_ROOT"
...
orb_require_repository_source() {
  orb_is_repository_source ||
    orb_die 'This command is available only from an OrbV source checkout.' 2
}
