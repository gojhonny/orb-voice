...
import type { OrboRealtimeSessionEndpoint } from './voice-model.types'
...
describe('talk/realtime-session-boundary', () => {
  ...
  it('snapshots a mutable URL and freezes only the copied endpoint configuration', () => {
    ...
    const input: OrboRealtimeSessionEndpoint = { endpoint, credentials: 'same-origin' }
  ...
  it('guards direct adapter construction as well as the native property', () => {
    ...
    const orb = document.createElement('orb-o') as OrboElement
