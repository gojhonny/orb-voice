import type { OrbuVoiceModel } from './voice-model.types'

/** Keep executable authorization and undeclared properties out of the public model. */
export function normalizeVoiceModel(
  value: OrbuVoiceModel | null | undefined
): Readonly<OrbuVoiceModel> | undefined {
  if (value === undefined || value === null) {
    return undefined
  }
  if (typeof value !== 'object') {
    throw new TypeError('Orbu voiceModel must be a provider configuration.')
  }

  let selection: OrbuVoiceModel
  switch (value.provider) {
    ...
    case 'openai-speech':
      if (!String(value.endpoint ?? '').trim()) {
        throw new TypeError('Orbu OpenAI speech selection requires an application endpoint.')
      }
      selection = {
        provider: value.provider,
    ...
    case 'openai-realtime':
      ...
      break
    default:
      throw new TypeError('Orbu voiceModel provider is not supported.')
  }
  ...
  for (const key of Object.keys(value)) {
    if (!(key in selection)) {
      ...
      if (Reflect.get(value, key) !== undefined) {
        throw new TypeError('Orbu voiceModel contains an unsupported option.')
      }
    }
  }
