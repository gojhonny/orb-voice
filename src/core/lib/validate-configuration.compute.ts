...
import type { OrboResolvedConfigurationSource } from '@core/config.types'
import { ORBO_DEFAULT_MOTION } from '@core/motion/default-motion.data'
import { ORBO_DEFAULT_SPEECH } from '@talk/default-speech.data'

import { cloneOrboConfigurationInput } from './clone-configuration.compute'

/** Validate a cloned input; diagnostics contain schema paths without supplied values. */
export function readOrboConfigurationSource(input: unknown): OrboResolvedConfigurationSource {
  const source = cloneOrboConfigurationInput(input)
  ...
  if (!Object.hasOwn(root, 'motion')) root.motion = cloneOrboConfigurationInput(ORBO_DEFAULT_MOTION)
  if (!Object.hasOwn(root, 'speech')) root.speech = cloneOrboConfigurationInput(ORBO_DEFAULT_SPEECH)
  ...
  choice(component.tagName, '$.component.tagName', ['orb-o'])
  ...
  if (!Object.hasOwn(appearance, 'byState')) {
    appearance.byState = cloneOrboConfigurationInput(ORBO_DEFAULT_APPEARANCE_BY_STATE)
  }
  ...
  return source as OrboResolvedConfigurationSource
}
  ...
  for (const state of states) {
    ...
    for (const layer of Object.keys(layers)) {
      ...
      const animate = record(
        ...
        `${layerPath}.animate`,
        [],
        ['--orbo-angle', 'opacity', 'rotate', 'scale', 'x', 'y']
      )
...
function animationScalar(value: unknown, path: string, property: string): void {
  if (property === 'opacity') {
    ...
    const units =
      property === 'rotate' || property === '--orbo-angle'
...
function invalid(path: string, reason: string): never {
  throw new TypeError(`Invalid Orbo configuration at ${path}: ${reason}.`)
}
