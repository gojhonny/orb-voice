...
for path in src/orbv.config.json .audits/configuration.inventory.md; do
...
for owner in \
  src/core/appearance/appearance.data.ts:ORBV_DEFAULT_APPEARANCE_BY_STATE \
  src/core/motion/default-motion.data.ts:ORBV_DEFAULT_MOTION \
  src/talk/default-speech.data.ts:ORBV_DEFAULT_SPEECH; do
  path=${owner%:*}
  binding=${owner#*:}
  if [ -f "$path" ] &&
    grep -F "export const $binding = deepFreezeOrbVConfiguration(" "$path" >/dev/null &&
...
if grep -E '"(byState|motion|speech)"[[:space:]]*:' src/orbv.config.json >/dev/null; then
...
if find src -type f -name '*.ts' ! -name '*.test.ts' -exec awk '
  ...
  function inspect(declaration, name, initializer, compact) {
    ...
    sub(/^[^=]*=[ \t]*/, "", initializer)
    sub(/[ \t]*;?[ \t]*$/, "", initializer)
    if (initializer ~ /^orbvConfiguration(\.[A-Za-z_][A-Za-z0-9_]*)+(\[[A-Z][A-Z0-9_]*\])?$/) return
    if (source == "src/core/config.data.ts" && name == "DEFAULT_ORBV_COLORS" &&
        initializer == "ORBV_PRESETS[DEFAULT_ORBV_PRESET]") return
    if (source == "src/factories/element-class.factory.ts" && name == "ELEMENT_CONSTRUCTORS" &&
        initializer == "new WeakMap<object, OrbVElementConstructor>()") return
    compact = initializer
    gsub(/[ \t\r\n]/, "", compact)
    if (source == "src/core/config.data.ts" && name == "ORBV_VOICE_DEFAULTS" &&
        compact ~ /^deepFreezeOrbVConfiguration\(\{([A-Za-z_][A-Za-z0-9_]*:orbvConfiguration(\.[A-Za-z_][A-Za-z0-9_]*)+,?)+\}\)$/) return
    if (source == "src/core/config.data.ts" && name == "config" &&
        compact ~ /^deepFreezeOrbVConfiguration\(\{([A-Z][A-Z0-9_]*,?)+\}\)$/) return
    reject(name " must derive directly from orbvConfiguration; see .audits/configuration.inventory.md")
  }
  FNR == 1 {
  ...
  FILENAME == "src/talk/default-speech.data.ts" { next }
  {
    ...
    if ((compatibility && match($0, /(const|let|var)[ \t]+[A-Za-z_][A-Za-z0-9_]*[ \t]*(:|=)/)) ||
      ...
      if (declaration ~ /= deepFreezeOrbVConfiguration\(\{[ \t]*$/) {
        pending = declaration
        aggregate = 1
      }
...
for path in src/core/config.data.ts src/core/motion/motion.data.ts src/element/element.data.ts src/talk/talk.data.ts; do
  if grep -F 'orbvConfiguration.' "$path" >/dev/null 2>&1; then
...
if grep -F 'orbvConfiguration.speech.talk' src/talk/talk.data.ts >/dev/null 2>&1 &&
  grep -F 'orbvConfiguration.speech.defaultTalkFlow' src/talk/talk.data.ts >/dev/null 2>&1 &&
...
if grep -F 'orbvConfiguration.speech.tokenPattern.source' src/talk/resolve-talk-text.compute.ts >/dev/null 2>&1 &&
  grep -F 'orbvConfiguration.speech.tokenPattern.flags' src/talk/resolve-talk-text.compute.ts >/dev/null 2>&1; then
