...
for path in src/orbu.config.json .audits/configuration.inventory.md; do
...
for owner in \
  src/core/appearance/appearance.data.ts:ORBU_DEFAULT_APPEARANCE_BY_STATE \
  src/core/motion/default-motion.data.ts:ORBU_DEFAULT_MOTION \
  src/talk/default-speech.data.ts:ORBU_DEFAULT_SPEECH; do
  path=${owner%:*}
  binding=${owner#*:}
  if [ -f "$path" ] &&
    grep -F "export const $binding = deepFreezeOrbuConfiguration(" "$path" >/dev/null &&
...
if grep -E '"(byState|motion|speech)"[[:space:]]*:' src/orbu.config.json >/dev/null; then
...
if find src -type f -name '*.ts' ! -name '*.test.ts' -exec awk '
  ...
  function inspect(declaration, name, initializer, compact) {
    ...
    sub(/^[^=]*=[ \t]*/, "", initializer)
    sub(/[ \t]*;?[ \t]*$/, "", initializer)
    if (initializer ~ /^orbuConfiguration(\.[A-Za-z_][A-Za-z0-9_]*)+(\[[A-Z][A-Z0-9_]*\])?$/) return
    if (source == "src/core/config.data.ts" && name == "DEFAULT_ORBU_COLORS" &&
        initializer == "ORBU_PRESETS[DEFAULT_ORBU_PRESET]") return
    if (source == "src/factories/element-class.factory.ts" && name == "ELEMENT_CONSTRUCTORS" &&
        initializer == "new WeakMap<object, OrbuElementConstructor>()") return
    compact = initializer
    gsub(/[ \t\r\n]/, "", compact)
    if (source == "src/core/config.data.ts" && name == "ORBU_VOICE_DEFAULTS" &&
        compact ~ /^deepFreezeOrbuConfiguration\(\{([A-Za-z_][A-Za-z0-9_]*:orbuConfiguration(\.[A-Za-z_][A-Za-z0-9_]*)+,?)+\}\)$/) return
    if (source == "src/core/config.data.ts" && name == "config" &&
        compact ~ /^deepFreezeOrbuConfiguration\(\{([A-Z][A-Z0-9_]*,?)+\}\)$/) return
    reject(name " must derive directly from orbuConfiguration; see .audits/configuration.inventory.md")
  }
  FNR == 1 {
  ...
  FILENAME == "src/talk/default-speech.data.ts" { next }
  {
    ...
    if ((compatibility && match($0, /(const|let|var)[ \t]+[A-Za-z_][A-Za-z0-9_]*[ \t]*(:|=)/)) ||
      ...
      if (declaration ~ /= deepFreezeOrbuConfiguration\(\{[ \t]*$/) {
        pending = declaration
        aggregate = 1
      }
...
for path in src/core/config.data.ts src/core/motion/motion.data.ts src/element/element.data.ts src/talk/talk.data.ts; do
  if grep -F 'orbuConfiguration.' "$path" >/dev/null 2>&1; then
...
if grep -F 'orbuConfiguration.speech.talk' src/talk/talk.data.ts >/dev/null 2>&1 &&
  grep -F 'orbuConfiguration.speech.defaultTalkFlow' src/talk/talk.data.ts >/dev/null 2>&1 &&
...
if grep -F 'orbuConfiguration.speech.tokenPattern.source' src/talk/resolve-talk-text.compute.ts >/dev/null 2>&1 &&
  grep -F 'orbuConfiguration.speech.tokenPattern.flags' src/talk/resolve-talk-text.compute.ts >/dev/null 2>&1; then
