...
[ "$adr_count" -ge 12 ] && pass "$adr_count ADRs" || fail 'expected at least 12 Orbu ADRs'
[ "$spec_count" -ge 15 ] && pass "$spec_count SPECs" || fail 'expected at least 15 Orbu SPECs'
[ "$rule_count" -ge 12 ] && pass "$rule_count rules" || fail 'expected at least 12 Orbu rules'
...
if grep -R -n -i -E "$legacy_name|$legacy_phrase" .agents .audits cli AGENTS.md README.md package.json >/dev/null 2>&1; then
  ...
  pass 'repository harness terminology is Orbu-specific'
fi
