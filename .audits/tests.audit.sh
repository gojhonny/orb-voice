...
for (const alias of ['test', 'test:watch', 'test:coverage', 'typecheck', 'build', 'check']) {
  if (!(alias in (pkg.scripts ?? {}))) console.log(`PASS  ${alias} is owned by OrbV rather than package scripts`)
  else { console.error(`FAIL  package script duplicates OrbV command: ${alias}`); failed = true }
}
if (readFileSync('cli/src/commands/test.sh', 'utf8').includes('vitest run')) {
  console.log('PASS  OrbV owns deterministic Vitest execution')
} else {
  console.error('FAIL  orbv test must run Vitest once by default')
  failed = true
}
if (failed) process.exit(1)
NODE
...
for ci_sensor in lint typecheck test build; do
  if grep -F "run: ./cli/orbv $ci_sensor" .github/workflows/ci.yml >/dev/null 2>&1; then
    pass "CI exposes OrbV $ci_sensor explicitly"
  else
    fail "CI must expose ./cli/orbv $ci_sensor as an explicit sensor step"
  fi
done
