...
if (pkg.bin?.['orbu'] === './cli/orbu' && Object.keys(pkg.bin).length === 1) pass('package exposes exactly the orbu binary')
else fail('package bin must expose only orbu -> ./cli/orbu')
...
if (scripts['pnpm:devPreinstall'] === './cli/orbu setup --launcher --bootstrap') {
  pass('local pnpm install provisions the managed Orbu launcher')
} else {
  fail('pnpm:devPreinstall must provision the managed Orbu launcher')
}
if (scripts.setup === './cli/orbu setup --launcher') pass('setup recovery bridge delegates to Orbu')
else fail('setup recovery bridge must delegate to Orbu launcher setup')
if (scripts.prepack === './cli/orbu check') pass('prepack delegates to complete Orbu check')
else fail('prepack must delegate to ./cli/orbu check')
...
if [ -x cli/orbu ] && /bin/sh -n cli/orbu; then
...
if grep -F 'orbu git pre-commit' .husky/pre-commit >/dev/null 2>&1; then printf 'PASS  pre-commit delegates to Orbu\n'; else printf 'FAIL  pre-commit must delegate to Orbu\n' >&2; exit 1; fi
if grep -F 'orbu git commit-message' .husky/commit-msg >/dev/null 2>&1; then printf 'PASS  commit-msg delegates to Orbu\n'; else printf 'FAIL  commit-msg must delegate to Orbu\n' >&2; exit 1; fi
