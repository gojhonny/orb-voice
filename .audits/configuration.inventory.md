...
| --- | --- | --- |
| `src/orbv.config.json` | `component` | Identity, states, attributes, size, speed and reduced-motion mode |
| `src/orbv.config.json` | `appearance` | Presets, color channels and attributes |
| `src/orbv.config.json` | `realtime` | Realtime model/voice, fetch policy, transport limits |
| `src/core/appearance/appearance.data.ts` | `ORBV_DEFAULT_APPEARANCE_BY_STATE` | State contrast and saturation |
| `src/core/motion/default-motion.data.ts` | `ORBV_DEFAULT_MOTION` | Full/reduced motion, easings and animated properties |
| `src/talk/default-speech.data.ts` | `ORBV_DEFAULT_SPEECH` | Model selector, empty talk/flow, token grammar, browser and TTS defaults |
...
| --- | --- |
| `core/config.data.ts` component/palette constants and `config` | `orbvConfiguration.component` and `.appearance` |
| `ORBV_VOICE_DEFAULTS` | `orbvConfiguration.speech` and `.realtime.openai` |
