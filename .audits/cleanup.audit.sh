...
cleanup_tmp=${TMPDIR:-/tmp}/orbu-cleanup-audit.$$
...
if (cd "$cleanup_tmp" && "$default_fixture/cli/orbu" cleanup) >"$cleanup_tmp/default.log" 2>&1; then
...
if "$default_fixture/cli/orbu" cleanup >"$cleanup_tmp/repeated.log" 2>&1; then
...
if "$options_fixture/cli/orbu" cleanup --dry-run >"$cleanup_tmp/dry.log" 2>&1 && \
...
for mode in unknown conflict reversed help; do
  ...
  option_status=0
  "$options_fixture/cli/orbu" cleanup "$@" >"$cleanup_tmp/option.log" 2>&1 || option_status=$?
  expected_status=2
...
if "$options_fixture/cli/orbu" cleanup --keep-dependencies >"$cleanup_tmp/keep.log" 2>&1 && \
...
if "$options_fixture/cli/orbu" clean --dependencies >"$cleanup_tmp/legacy.log" 2>&1 && \
...
failure_status=0
PATH="$cleanup_tmp/failing-bin:$PATH" "$failure_fixture/cli/orbu" cleanup >"$cleanup_tmp/failure.log" 2>&1 || failure_status=$?
...
if "$failure_fixture/cli/orbu" cleanup >"$cleanup_tmp/index.log" 2>&1; then
...
if NO_COLOR=1 PATH="$cleanup_tmp/minimal-bin" "$minimal_fixture/cli/orbu" cleanup >"$cleanup_tmp/minimal.log" 2>&1 && \
...
published_status=0
"$cleanup_tmp/published/cli/orbu" cleanup >"$cleanup_tmp/published.log" 2>&1 || published_status=$?
if [ "$published_status" -eq 2 ] && \
  grep -F 'only from an Orbu source checkout' "$cleanup_tmp/published.log" >/dev/null; then
