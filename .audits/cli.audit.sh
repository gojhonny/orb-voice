...
for executable in cli/orbo cli/src/orb.sh cli/src/commands/*.sh cli/src/core/shell-syntax.sh; do
  if [ -x "$executable" ]; then pass "$executable is executable"; else fail "$executable is not executable"; fi
done

shell_failures=0
for shell_file in cli/orbo cli/src/*.sh cli/src/core/*.sh cli/src/commands/*.sh; do
  if ! /bin/sh -n "$shell_file"; then
    printf 'FAIL  invalid shell syntax: %s\n' "$shell_file" >&2
    shell_failures=$((shell_failures + 1))
  fi
done
if [ "$shell_failures" -eq 0 ]; then pass 'all Orbo shell files pass /bin/sh -n'; else failures=$((failures + shell_failures)); fi
...
if find cli .agents .audits .husky .github README.md AGENTS.md package.json .lintstagedrc.json \
  ...
  pass 'CLI naming is consistently Orbo'
fi
...
if grep -R -n -i -E 'k8s|container orchestrator|workspace task graph|product changelog|environment template' cli >/dev/null 2>&1; then
  ...
  pass 'CLI scope is specific to the Orbo library'
fi
...
for command in bootstrap setup doctor cleanup lint typecheck test build audit check help 'git setup' 'git doctor' 'git pre-commit' 'git commit-message' 'git lint' 'git version-check'; do
  if TERM=dumb ./cli/orbo help | grep -F "$command" >/dev/null 2>&1; then
...
if (scripts['pnpm:devPreinstall'] !== './cli/orbo setup --launcher --bootstrap') {
  console.error('FAIL  pnpm:devPreinstall must provision the managed Orbo launcher')
  process.exit(1)
}
if (scripts.setup !== './cli/orbo setup --launcher') {
  console.error('FAIL  setup script must delegate to Orbo launcher setup')
  process.exit(1)
}
if (scripts.prepack !== './cli/orbo check') {
  console.error('FAIL  prepack must delegate to Orbo check')
  process.exit(1)
}
...
if (pkg.bin?.['orbo'] !== './cli/orbo' || Object.keys(pkg.bin).length !== 1) {
  console.error('FAIL  package must expose exactly one orbo binary')
  process.exit(1)
}
console.log('PASS  source install provisions Orbo while consumer lifecycle hooks stay clean')
NODE

if grep -F '# managed-by: orbo' cli/src/commands/setup-launcher.sh >/dev/null 2>&1; then
  pass 'launcher uses the Orbo-managed marker'
else
  fail 'launcher marker is not Orbo-specific'
fi

if grep -E 'pnpm exec[[:space:]]+orbo|npm exec --[[:space:]]+orbo' README.md cli/readme.md >/dev/null 2>&1; then
  fail 'active documentation requires a package-manager executable runner for Orbo'
else
  pass 'active documentation uses direct orbo commands'
fi
...
orb_tmp=${TMPDIR:-/tmp}/orbo-cli-audit.$$
trap 'rm -rf "$orb_tmp"' 0 1 2 15
mkdir -p "$orb_tmp/bin" "$orb_tmp/package/cli" "$orb_tmp/project" "$orb_tmp/direct-bin"
ln -s "$ROOT/cli/orbo" "$orb_tmp/bin/orbo"
if "$orb_tmp/bin/orbo" --version | grep -Fx "orbo $(node -p "require('./package.json').version")" >/dev/null 2>&1; then
  pass 'package-manager style symlink resolves the real CLI location'
else
  fail 'CLI entry point fails through a symlink'
fi

if ORB_BIN_DIR="$orb_tmp/direct-bin" CI= ./cli/orbo setup --launcher --bootstrap >/dev/null 2>&1 &&
   PATH="$orb_tmp/direct-bin:$PATH" orbo --version | grep -Fx "orbo $(node -p "require('./package.json').version")" >/dev/null 2>&1; then
  pass 'managed launcher exposes orbo directly on PATH'
else
  fail 'managed launcher does not expose a direct orbo command'
fi
...
{
  "name": "orbo-consumer-audit",
  "private": true,
  "packageManager": "pnpm@10.32.1"
}
JSON
if (cd "$orb_tmp/project" && "$orb_tmp/package/cli/orbo" --dry-run) | grep -F 'pnpm add orbo-voice@' >/dev/null 2>&1; then
...
if (cd "$orb_tmp/project" && "$orb_tmp/package/cli/orbo") >/dev/null 2>&1; then
...
for orb_history_command in lint commits; do
  ...
  if "$orb_history/cli/orbo" git "$orb_history_command" --last >"$orb_tmp/history.log" 2>&1; then
  ...
  for orb_history_mode in last range; do
    ...
    if "$orb_history/cli/orbo" git "$orb_history_command" "$@" >"$orb_tmp/history.log" 2>&1; then
