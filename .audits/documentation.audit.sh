...
for image in \
  assets/images/orbu-tagline.svg \
...
tagline_line=$(grep -n -m1 'assets/images/orbu-tagline.svg' README.md | cut -d: -f1 || true)
...
for token in \
  ...
  'github/actions/workflow/status/gojhonny/orbu/ci.yml' \
  'img.shields.io/npm/v/orbu'
do
...
if awk '
  BEGIN {
    ...
    expected[2] = "<a href=\"https://www.npmjs.com/package/orbu\"><strong>npm</strong></a>"
...
# ---------------------------------------------------------------------------
# Engineering CLI documentation: direct Orbu is canonical
# ---------------------------------------------------------------------------
...
if grep -E 'pnpm exec[[:space:]]+orbu|npm exec --[[:space:]]+orbu' README.md cli/readme.md >/dev/null 2>&1; then
  ...
  pass 'active documentation uses orbu directly for engineering commands'
fi

if grep -E 'pnpm (orbu|check|version:check)' README.md cli/readme.md >/dev/null 2>&1; then
  fail 'documentation retains removed package-script command aliases'
else
  pass 'documentation keeps Orbu as the engineering command surface'
fi
